...
async function run() {
  ...
  const contractFactory = await getContractFactory("psp22_emitable_contract", signer);
  ...
  const contract = await contractFactory.deploy("new", "lolname", "loldescription",37, {
